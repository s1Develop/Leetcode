...
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        ...
        # 먼저 hashtable에 headA를 전부 넣고
        while headA != None:
            hash_table[headA] = headA.val
            headA = headA.next

        # headB linked list안에 headA랑 intersection이 있으면 바로 그애를 return한다
        while headB != None:
            if headB not in hash_table:
                headB = headB.next
            else:
                return headB

        # 끝까지 못찾은거니까 return None
        return None
